package com.kael.login.qzone.gui;
...
public class Main extends JFrame {
    ...
    private static int qzone_js_ver = 10114;
    public Main() {
        super();
...
//            HttpGet httpGet = new HttpGet();
            ...
            jLabel_VerifyCode.setIcon(new ImageIcon(Utils.requestGet("http://captcha.qq.com/getimage?uin=" + jTextField_UID.getText() + "&aid=549000912&" + Math.random())));
...
//                        appendJTextArea("Http Request：" + httpGet.getURI());                        
                        String responseBody = new String(Utils.requestGet(url));
//                        String ptuiStr = Utils.findPattern("pt.ptui\\=\\{(.+)}", responseBody);
//                        ptuiStr.subSequence(0, ptuiStr.indexOf(";"));
//                        appendJTextArea("Http Response: " + ptuiStr);
						//PTUI ptui = JSON.parseObject(ptuiStr.replace("pt.ptui=", "").replace("encodeURIComponent(", "").replace(")", ""), PTUI.class);
                        PTUI ptui = parse(responseBody);
                        if(ptui == null) {
                        ...
                        if (jCheckBox.isSelected()) {
                            ...
                            String uri = "http://check.ptlogin2.qq.com/check?regmaster=&uin=" + jTextField_UID.getText() + "&appid=549000912&js_ver="+qzone_js_ver+"&js_type=1&login_sig=" + ptui.getLogin_sig() + "&u1=http%3A%2F%2Fqzs.qq.com%2Fqzone%2Fv5%2Floginsucc.html%3Fpara%3Dizone&r=" + Math.random();
                            //appendJTextArea("Http Request：" + httpGet.getURI());
                            responseBody = new String(Utils.requestGet(uri));
                            appendJTextArea("Http Response: " + responseBody);
                            String[] params = Utils.findPattern(Utils.FUNCTION_PATTERN, responseBody).replace("'", "").split(",");
                        ...
                        // 第三步登陆空间
                        String uri = "http://ptlogin2.qq.com/login?u=" + jTextField_UID.getText() + "&p=" + Utils.getPwd(uin == null ? Utils.uin2hex(jTextField_UID.getText()) : uin, new String(jTextField_PWD.getPassword()).trim(), verifyCode) + "&verifycode=" + verifyCode + "&aid=549000912&u1=http%3A%2F%2Fqzs.qq.com%2Fqzone%2Fv5%2Floginsucc.html%3Fpara%3Dizone&h=1&ptredirect=0&ptlang=2052&daid=5&from_ui=1&dumy=&low_login_enable=0&regmaster=&fp=loginerroralert&action=2-21-1385452444158&mibao_css=&t=1&g=1&js_ver="+qzone_js_ver+"&js_type=1&login_sig=" + ptui.getLogin_sig() + "&pt_rsa=0&pt_qzone_sig=1";
    ...
    private PTUI parse(String resp){
    	resp = Utils.findPattern("pt.ptui\\=\\{(.+)}", resp);
    	resp = resp.substring(resp.indexOf("=") + 1,resp.indexOf(";"));
		resp = resp.replace("pt.ptui=", "").replace("encodeURIComponent(", "").replace(")", "");
		return JSON.parseObject(resp, PTUI.class);
    }
